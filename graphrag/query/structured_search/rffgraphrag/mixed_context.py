...
class RFFGContext(LocalContextBuilder):
    ...
    def __init__(
        ...
        graphdb_driver: Any = None,
        get_research_context: bool = False,
    ...
    def build_context(
        ...
        k_shortest_paths: int = 2,
        path_depth: int = 10,
        top_k_paths: int = 30,
        get_relevant_entities_only: bool = True,
        selected_entities: list[str] | None = None,
        **kwargs: dict[str, Any],
    ) -> ContextBuilderResult:

        # TODO:get_relevant_entities_only parameter is unnecessary remove later
        if selected_entities:
            get_relevant_entities_only = False
        """
        Build data context for rffg search prompt.
        ...
        if get_relevant_entities_only:
            ...
            full_text_searched_entities = full_text_search(
                query=query,
                all_entities_dict=self.entities,
                k=top_k_mapped_entities,
                graphdb_driver=self.graphdb_driver,
            )

            # can have duplicates
            selected_entities.extend(full_text_searched_entities)
            unique_selected_entities = list()
            found_path = set()
            
            for i in selected_entities: 
                if i.id not in found_path:
                    found_path.add(i.id)
                    unique_selected_entities.append(i)
            
            selected_entities = unique_selected_entities
            

            final_context.append("--------- Entities Found --------\nID | Title\n"+"\n".join([entity.id +" | " + entity.title for entity in selected_entities]))
            # print("".join([entity.id +" | " + entity.title for entity in selected_entities]))
            return ContextBuilderResult(
            context_chunks="\n\n".join(final_context),
            context_records=final_context_data,
            )
        len_entities = len(selected_entities)
        selected_entities_id = selected_entities
        selected_entities = list(map(lambda x: get_entity_by_id(self.entities, x), selected_entities)) # get_entity_by_id
        all_paths = list()
        # Find paths between selected entities
        for a in range(len_entities):
            source = selected_entities[a]
            for b in range(a+1, len_entities):
                target = selected_entities[b]
                if a==b:
                    continue
                shortest_path_query = f"""
                    MATCH p = SHORTEST {k_shortest_paths} (n:LABEL)-[:RELATION*1..{path_depth}]-(m:LABEL)
                    where n.name = $source and m.name = $target 
                    RETURN p, length(p) AS result LIMIT {top_k_paths} 
                """
                # breakpoint()
                shortest_paths = self.graphdb_driver.execute_query(shortest_path_query, source = source.title, target=target.title)[0]
        ...
        print(len(all_paths))
        paths_context = list[str]()
        new_entities_in_path = set()
        paths_context.append("--------- Paths Found --------\n")
        
        for path in all_paths:
            entities_in_path = []
            relationships_in_path = []
            for relation in path[0].relationships:
                new_entities_in_path.add(relation.nodes[0]._properties["id"])
                new_entities_in_path.add(relation.nodes[1]._properties["id"])
                entities_in_path.append((relation.nodes[0]._properties["name"],relation.nodes[1]._properties["name"]))
                relationships_in_path.append(relation._properties["description"]) 
            
            # TODO: Fix Paths
            path_context = f"Path found from {path[0].start_node._properties['name']} to {path[0].end_node._properties['name']} of length {path[1]}\n"
            last_entities = ""
            for entities,relationship  in zip(entities_in_path, relationships_in_path):
                if entities[0] == last_entities:
                    path_context += f"--{relationship} --> {entities[1]}"
                    last_entities = entities[1]
                elif entities[1] == last_entities:
                    path_context += f"<--{relationship} -- {entities[0]}"
                    last_entities = entities[0]
                else:
                    path_context += f"{entities[0]} -- {relationship} --> {entities[1]}\n"
                    last_entities = entities[1]
                
                # path_context += f"{entities[0]} -- {relationship} --> {entities[1]}\n"
            path_context+="\n"
            paths_context.append(path_context)
            
        # TODO: convert new entities into entity objects
        for i in new_entities_in_path:
            if i not in selected_entities_id:
                selected_entities.append(get_entity_by_id(self.entities, i)) 

        # print(selected_entities)

        
        final_context.append(''.join(paths_context))
        if conversation_history:
        ...
        local_tokens = max_context_tokens
    ...
    def _build_local_context(
        ...
        # one build
        added_entities.extend(selected_entities)
